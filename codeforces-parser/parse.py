...
TEMPLATE = os.path.join(os.path.expanduser('~'), 'Documents', 'codeforces-parser', 'template.cpp')
COMPILE_CMD='g++ -g -std=c++11 $DBG'
SAMPLE_INPUT='input'
...
class CodeforcesProblemParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        if tag == 'div':
            ...
            elif attrs == [('class', 'problem-statement')]:
                self.parse_statement = True
                self.testcase = open('%s/problem' % (self.folder), 'wb')
                self.start_copy = True
            elif self.parse_statement and attrs == [('class', 'sample-tests')]:
                self.testcase.close()
                self.parse_statement = False
                self.start_copy = False
            elif self.parse_statement and attrs == [('class', 'property-title')]:
                self.is_property = 1
            else:
                if self.start_copy:
                    self.testcase.write('\n'.encode('utf-8'))
            ...
            if self.testcase != None:
                self.start_copy = True
        elif self.parse_statement and tag == 'p':
            self.testcase.write('\n'.encode('utf-8'))
    ...
    def handle_data(self, data):
        if self.start_copy:
            if self.parse_statement:
                if self.is_property == 2:
                    self.testcase.write(': '.encode('utf-8'))
                    self.is_property = 0
                if self.is_property:
                    self.is_property += 1
...
def generate_test_script(folder, num_tests, problem):
    with open(folder + 'test.sh', 'w') as test:
        ...
        test.write(
            ...
            '    else\n'
            '        if diff --brief -w $MY_NAME$test_case $OUTPUT_NAME$test_case; then\n'
...
def main():
    ...
    for index, problem in enumerate(content.problems):
        ...
        call(['mkdir', '-p', folder])
        shutil.copyfile(TEMPLATE, '%s/%s/%s.%s' % (contest, problem, problem, TEMPLATE.split('.')[1]))
